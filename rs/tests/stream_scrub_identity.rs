...
fn identity_hook_matches_no_hook() {
    for t in TYPES {
        ...
        let hooked_fp = record_fixed(hooked.path(), t, Some(identity_scrub()));
...
fn identity_hook_replays_across_the_hook_boundary() {
    for t in TYPES {
        ...
        record_fixed(with_hook.path(), t, Some(identity_scrub()));
        replay_fixed(with_hook.path(), t, None);

        let without = TempDir::new().unwrap();
        record_fixed(without.path(), t, None);
        replay_fixed(without.path(), t, Some(identity_scrub()));
    }
}
...
fn identity_derived_identity_equals_raw() {
    for mode in [Mode::Record, Mode::Replay] {
        ...
        let s = Session::with_stream_scrub(mode, FileCassette::new(dir.path()), identity_scrub());
